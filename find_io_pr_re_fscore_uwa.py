...
lib = CDLL("/home/username/darknet_dir/libsoFile.so", RTLD_GLOBAL)
...
def detect(net, meta, image, thresh=.2, hier_thresh=.5, nms=.45):
    ...
    return res
specie_list=['Abudefduf_bengalensis',
           ...
           'Thalassoma_lunare',
           'Other_Other']    
net = load_net("/home/user/darknet_dir/cfg/yolov3_uwa.cfg", "/home/user/darknet_dir/uwa_yolov3/weightFile.weights", 0)
meta = load_meta("/home/ahsanjalal/darknet_alex/cfg/uwa.data")
total_gt_count=0
total_det_cunt=0
TP=0
FP=0
val_list=open('/home/user/uwa/validation_data.txt')
...
for img_name in val_lines:
    ...
    if r:
        ...
        for fish_info in r:
                ...
                obj_arr.append(tmp)
                if(xmax_det > gt_width):
                    xmax_det=gt_width
              
                if(ymax_det > gt_height):
                    ymax_det=gt_height
                ...
                for line_gt in gt_text:
                    ...
                    if(xb>xa and yb>ya):
                        ...
                        if(float(area_inter)/area_min>=0.5): # IOMin\
                           ...
                           FP+=1
        if(match_flag==0): # if no overlapp of detected obj from any GT of that frame
            FP+=1
            print('false count is {}'.format(FP))
   ...
   f.write(xml_content)
   f.close()
   cv2.imwrite(join(saving_dir,video_file,img_file),img_rgb)                        
