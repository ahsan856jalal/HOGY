...
"lunare",
"Other"]    
net = load_net("/home/user/darknet/cfg/res50.cfg", "/home/user/darknet/weightfile.weights", 0)
meta = load_meta("/home/user/darknet/cfg/dataFile.data")
...
total_gt_count=0
total_det_count=0
# make your own directories paths 
...
for val_txt1 in val_lines:
    ...
    gt_height=img_rgb.shape[0]
    gt_width=img_rgb.shape[1]
    gmm_width=gt_width
    gmm_height=gt_height
    ...
    if(os.path.exists(yolo_txt_path) or (os.path.exists(gmm_optical_txt_path))):
        ...
        if(len(yolo_txt)==0 and len(gmm_optical_txt)!=0): # only gmm detections
            ...
            for line_gt in gt_lines:
                    ...
                    img=cv2.rectangle(img_rgb,(xmin_gt,ymin_gt),(xmax_gt,ymax_gt),(0,0,0),2)
                    if(xmin_gt<0):
                        xmin_gt=0
                    if(ymin_gt<0):
                        ymin_gt=0
            ...
            for gmm_txt1 in yolo_txt:
                    ...
                    # now calculating IOMin 
                        ...
                        if(xb>xa and yb>ya):
                            ...
                            area_min=min(area_gt,area_pred)
                            area_union=area_pred+area_gt-area_inter
                            if(float(area_inter)/area_min>=0.5):#area_union=0.3
                                match_flag+=1
...
#                                img=cv2.rectangle(img_rgb,(xmin_gmm,ymin_gmm),(xmax_gmm,ymax_gmm),(127,89,224),2)
...
#                    img=cv2.rectangle(img_rgb,(xmin_gt,ymin_gt),(xmax_gt,ymax_gt),(255,12,0),2)
            ...
            for gmm_txt1 in gt_lines:
                ...
                xmax_gmm = x_gmm + w_gmm/2
                ymax_gmm = y_gmm + h_gmm/2  
                img=cv2.rectangle(img_rgb,(xmin_gmm,ymin_gmm),(xmax_gmm,ymax_gmm),(0,0,0),2)
                if(xmin_gmm<0):
                    xmin_gmm=0
                if(ymin_gmm<0):
                    ymin_gmm=0
                ...
                for line_gt in new_optical_gmnm_yolo_txt1:
                    count_gt_line+=1
                    line_gt1 = line_gt.rstrip()
                    coords=line_gt1.split(' ')
                    label_gt=int(coords[0])
